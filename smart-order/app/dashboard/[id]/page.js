...
function GaugeWidget({ widget, device }) {
  ...
  const unit = widget.config?.unit ?? "";
  const color = widget.config?.color ?? "#4ade80";
...
function WidgetConfigModal({ widget, devices, onSave, onClose }) {
  const [cfg, setCfg] = useState({
    ...
    color:      widget.config?.color      ?? "#4ade80",
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "24px" }}>
      <div style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: "20px", width: "100%", maxWidth: "360px", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              ...
              />
              {["#4ade80","#60a5fa","#f87171","#fb923c","#a78bfa","#facc15"].map((c) => (
        ...
        <div style={{ display: "flex", gap: "8px", marginTop: "16px" }}>
          <button onClick={save} style={{ flex: 1, padding: "9px 0", fontSize: "13px", fontWeight: 600, background: "var(--accent-subtle)", color: "var(--accent)", border: "1px solid rgba(74,222,128,0.25)", borderRadius: "var(--radius-sm)", cursor: "pointer" }}>
            Simpan
          </button>
...
export default function ProjectDashboardPage({ params }) {
  ...
  function addWidget(type) {
    ...
    const newW = {
      ...
      w: def.defaultW,
      h: def.defaultH,
      config: { pin: "temperature", deviceName: devices[0]?.deviceName ?? "", color: "#4ade80" },
    };
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg-primary)" }}>
      ...
      <div style={{ padding: "12px 24px", borderBottom: "1px solid var(--border)", background: "var(--bg-secondary)", display: "flex", alignItems: "center", gap: "12px" }}>
        ...
        <div style={{ display: "flex", gap: "6px" }}>
          {devices.map((d) => (
            <span key={d.id} style={{ fontSize: "10px", padding: "3px 9px", borderRadius: "20px", background: d.status === "online" ? "var(--accent-subtle)" : "var(--bg-tertiary)", color: d.status === "online" ? "var(--accent)" : "var(--text-muted)", border: `1px solid ${d.status === "online" ? "rgba(74,222,128,0.2)" : "var(--border)"}` }}>
              {d.deviceName}
            </span>
          ))}
        </div>
        ...
        {editMode && (
          <button onClick={() => setAddMenu((v) => !v)}
            style={{ background: "var(--accent-subtle)", color: "var(--accent)", border: "1px solid rgba(74,222,128,0.25)", borderRadius: "var(--radius-sm)", padding: "6px 14px", fontSize: "12px", fontWeight: 600, cursor: "pointer" }}
          >
            + Widget
          </button>
        )}
